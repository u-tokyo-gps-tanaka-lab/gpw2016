...
namespace DigitalCurling{
    namespace FastSimulator{
        ...
        std::array<fpn_t, (1 << FV_FR_TABLE_RESOLUTION) + 4> FV_FRTable;
        std::array<fpn_t, (1 << FR_FV_SR_TABLE_RESOLUTION) + 4> FR_FVTableSR;
        std::array<fpn_t, (1 << FR_FV_TABLE_RESOLUTION) + 4> FR_FVTable;
        std::array<std::array<fpn_t, 2>, (1 << FR_CZ_TABLE_RESOLUTION) + 4> FR_CZTable;

        /**************************前計算データへのアクセス*************************/
        
        /*fpn_t FVtoFR(fpn_t fv){ // V -> R 変換
            int fid = uint32(fv * FV_FR_TABLE_INVSTEP);
            fpn_t frac = fv - ((fpn_t)fid) * FV_FR_TABLE_STEP;
            ASSERT(0 <= fid && fid<(1 << FV_FR_TABLE_RESOLUTION) + 4, cerr << fv / FV_TABLE_MAX << endl;);
            ASSERT(0 <= frac && frac <= FV_FR_TABLE_STEP, cerr << frac << endl;);
            return (FV_FRTable[fid] * (FV_FR_TABLE_STEP - frac) + FV_FRTable[fid + 1] * frac) * FV_FR_TABLE_INVSTEP;
        }

        fpn_t FRtoFV(fpn_t fr){ // R -> V 変換
            if (fr > FR_SR_TABLE_MAX){
                int fid = int(fr * FR_FV_TABLE_INVSTEP);
                fpn_t frac = fr - ((fpn_t)fid) * FR_FV_TABLE_STEP;
                ASSERT(0 <= fid && fid<(1 << FR_FV_TABLE_RESOLUTION) + 4, cerr << fr / FR_TABLE_MAX << endl;);
                ASSERT(0 <= frac && frac <= FR_FV_TABLE_STEP, cerr << frac << endl;);
                return (FR_FVTable[fid] * (FR_FV_TABLE_STEP - frac) + FR_FVTable[fid + 1] * frac) * FR_FV_TABLE_INVSTEP;
                //return (FR_FVTable[fid] * sqrt(FR_FV_TABLE_STEP - frac) + FR_FVTable[fid + 1] * sqrt(frac))/(sqrt(FR_FV_TABLE_STEP - frac)+sqrt(frac));
            }else{
                // Rが小さい時のみ、精度向上の為別テーブルにする
                // R -> V が上に凸な関数であるためRが小さいときには線形補完の誤差が大きくなるため
                int fid = int(fr * FR_FV_SR_TABLE_INVSTEP);
                fpn_t frac = fr - ((fpn_t)fid) * FR_FV_SR_TABLE_STEP;
                //ASSERT(0 <= fid && fid<(1 << FR_FV_TABLE_RESOLUTION) + 4, cerr << fr / FR_TABLE_MAX << endl;);
                //ASSERT(0 <= frac && frac <= FR_FV_TABLE_STEP, cerr << frac << endl;);
                return (FR_FVTableSR[fid] * (FR_FV_SR_TABLE_STEP - frac) + FR_FVTableSR[fid + 1] * frac) * FR_FV_SR_TABLE_INVSTEP;
                ///(sqrt(FR_FV_SR_TABLE_STEP - frac)+sqrt(frac));
            }
        }*/
        
        fpn_t FVtoFR(fpn_t fv){ // V -> R 変換
            return FDR_FV_FR * fv * fv;
        }
        
        fpn_t FRtoFV(fpn_t fr){ // R -> V 変換
            return sqrt(fr / FDR_FV_FR);
        }
        ...
        //fpn_t FVtoFX(fpn_t fv){FV_FAllTable[FVtoID(fv)].x;}
        //fpn_t FVtoFY(fpn_t fv){FV_FAllTable[FVtoID(fv)].y;}
        //fpn_t FVtoFTh(fpn_t fv){FV_FAllTable[FVtoID(fv)].th;}

        fpn_t ID_FRACtoFX(uint32 id, fpn_t frac){
            return (FV_FAllTable[id].x * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].x * frac) * FV_TABLE_INVSTEP;
        }
        fpn_t ID_FRACtoFY(uint32 id, fpn_t frac){
            return (FV_FAllTable[id].y * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].y * frac) * FV_TABLE_INVSTEP;
        }
        fpn_t ID_FRACtoFTh(uint32 id, fpn_t frac){
            return (FV_FAllTable[id].th * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].th * frac) * FV_TABLE_INVSTEP;
        }

        // メインテーブルの利用
        fpn_t FVtoFX(fpn_t fv){
            uint32 id = FVtoID(fv); fpn_t frac = FVtoFRAC(fv);
            return (FV_FAllTable[id].x * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].x * frac) * FV_TABLE_INVSTEP;
        }
        ...
        void initV_R(){ // V_R変換初期化
            ...
            FV_FRTable[0] = 0;
            pfpn_t tm = 0; // 逆算時刻
            for (int i = 1; i < FV_FRTable.size(); ++i){
                // V_Rテーブルに現時点での値を入れていく
                pfpn_t line = i * PFV_FR_TABLE_STEP;
                pfpn_t fv;
                while (1){
                ...
                pfpn_t frac;
                if (fv != lfv){
                    frac = (line - lfv) / (fv - lfv); // lfv < line <= fv
                }
                else{
                    frac = static_cast<pfpn_t>(0.5);
                }
                ASSERT(0 <= frac && frac <= 1.0, cerr << double(lfv) << "->" << double(fv) << "(" << double(line) << ")" << endl;);
                FV_FRTable[i] = XYtoR(lmst.x, lmst.y) * ((pfpn_t)1 - frac) + XYtoR(tmst.x, tmst.y) * frac;
            }*/
        }
        void initR_V(){ // R_V変換初期化
            ...
            FR_FVTable[0] = 0;
            pfpn_t tm = 0; // 逆算時刻
            for (auto i = 1; i < FR_FVTable.size(); ++i){
                // V_Rテーブルに現時点での値を入れていく
                pfpn_t line = i * PFR_FV_TABLE_STEP;
                pfpn_t fr;
                while (1){
                    fr = XYtoR(mst.x, mst.y);
                    if (fr >= line){ break; }
                    Simulator::invStepSolo<pfpn_t>(&mst, step);
                    tm += step;
                }
                ...
                lmst = mst;
                Simulator::stepSolo<pfpn_t>(&lmst, step);
                pfpn_t lfr = XYtoR(lmst.x, lmst.y);
                pfpn_t frac;
                if (fr != lfr){
                    frac = (line - lfr) / (fr - lfr); // lfr < line <= fr
                }
                else{
                    frac = static_cast<pfpn_t>(0.5);
                }
                ASSERT(0 <= frac && frac <= 1.0, cerr << double(lfr) << "->" << double(fr) << "(" << double(line) << ")" << endl;);
                FR_FVTable[i] = XYtoR(lmst.vx(), lmst.vy()) * ((pfpn_t)1 - frac) + XYtoR(tmst.vx(), tmst.vy()) * frac;
                //CERR << (FR_FVTable[i] - FR_FVTable[i - 1]) << endl;
            }*/
        }
        void initR_VinSR(){ // R_V変換(短レンジ)初期化
            /*constexpr pfpn_t step = static_cast<pfpn_t>(1) / static_cast<pfpn_t>(1500000);

            fMStone<pfpn_t> mst(0, 0, 0, 0, 0), lmst, tmst;
            FR_FVTableSR[0] = 0;
            pfpn_t tm = 0; // 逆算時刻
            for (auto i = 1; i < FR_FVTableSR.size(); ++i){
                // V_Rテーブルに現時点での値を入れていく
                pfpn_t line = i * PFR_FV_SR_TABLE_STEP;
                pfpn_t fr;
                while (1){
                    fr = XYtoR(mst.x, mst.y);
                    if (fr >= line){ break; }
                ...
                ASSERT(0 <= frac && frac <= 1.0, cerr << double(lfr) << "->" << double(fr) << "(" << double(line) << ")" << endl;);
                FR_FVTableSR[i] = XYtoR(lmst.vx(), lmst.vy()) * ((pfpn_t)1 - frac) + XYtoR(tmst.vx(), tmst.vy()) * frac;
                //CERR<<FR_FVTableSR[i]-FR_FVTableSR[i-1]<<endl;
            }*/
        }
        ...
        void init(){
            ...
            thread_t th0(initV_R);
            thread_t th1(initR_V);
            thread_t th2(initALL);
            thread_t th3(initR_VinSR);
        ...
        fpn_t calcVThetaOnGoal(const mst_t& mst){
            return mst.th() - FVtoFTh(mst.v());
        }*/
