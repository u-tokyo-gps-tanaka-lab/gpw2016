...
#ifndef DCURLING_SIMULATION_INTSIMULATOR_H_
#define DCURLING_SIMULATION_INTSIMULATOR_H_

namespace DigitalCurling{
    namespace IntSimulator{
        
        //型
        typedef int simint_t;
        typedef long long sim2int_t;
        typedef unsigned int simuint_t;
        typedef unsigned long long sim2uint_t;
        
        //定数
        constexpr int I_FRIC_STONE = 1;
        constexpr int IV_RESOLUTION = 21;//velocity
        constexpr int IR_RESOLUTION = 45;//length
        constexpr int IF_RESOLUTION = 8;//fraction
        
        constexpr int I16COS_CURL_MAX = static_cast<int>(cos(FastSimulator::FTHETA_CURL_MAX) * (1<<16));//停止位置への回転最大角度
        constexpr int I16SIN_CURL_MAX = static_cast<int>(sin(FastSimulator::FTHETA_CURL_MAX) * (1<<16));
        constexpr int I16COS_CURL = static_cast<int>(cos(FastSimulator::FTHETA_CURL) * (1<<16));//停止位置への回転角度
        constexpr int I16SIN_CURL = static_cast<int>(sin(FastSimulator::FTHETA_CURL) * (1<<16));
        constexpr sim2int_t IR_COLLISION = 256;//衝突閾値距離
        constexpr int I16_SLID_STONES = static_cast<int>(FastSimulator::F_SLID_STONES * (1<<16));//石どうしの滑り係数
        
        //浮動小数点型との変換
        //変換定数
        constexpr fpn_t I64R_FR_STEP=(FR_HOUSE_RAD * 32) / (1LL<<IR_RESOLUTION);
        constexpr fpn_t I32V_FV_STEP=(FV_MAX+2.0) / (1LL<<IV_RESOLUTION);
        
        template<typename float_t=fpn_t>
        constexpr float_t I64RtoFR(sim2int_t ir){return static_cast<float_t>(ir)*I64R_FR_STEP;}
        template<typename float_t=fpn_t>
        constexpr sim2int_t FRtoI64R(float_t fr){return static_cast<sim2int_t>(fr/I64R_FR_STEP);}
        
        constexpr fpn_t I32VtoFV(simint_t ir){return static_cast<fpn_t>(ir)*I32V_FV_STEP;}
        constexpr simint_t FVtoI32V(fpn_t fr){return static_cast<simint_t>(fr/I32V_FV_STEP);}
        
        template<typename float_t=fpn_t>
        constexpr float_t ITtoFT(simint_t it){return static_cast<float_t>(it*I_FRIC_STONE)
            *static_cast<float_t>(I32V_FV_STEP)
            /static_cast<float_t>(F_FRIC_RINK_STONE);}
        
        //r -> all conversion
        struct ISimuData{
            //原点(0,0)で,(0,1)方向に静止する石の、
            //それぞれの地点での状態
            sim2int_t x, y;//position
            int cos,sin;//direction of movement
            
            void set(sim2int_t ax, sim2int_t ay, int acos, int asin){
                x=ax; y=ay; cos=acos; sin=asin;
            }
        };
        
        constexpr int IR_TABLE_RESOLUTION = 18;
        constexpr sim2uint_t IR_TABLE_STEP = static_cast<sim2uint_t>(1)<<(IR_RESOLUTION - IR_TABLE_RESOLUTION);
        
        constexpr uint32 IRtoID(sim2uint_t i){
            return (uint32)(i >> IR_TABLE_RESOLUTION);
        }
        constexpr uint32 IRtoFRAC(sim2uint_t i){
            return (((uint32)i) % (uint32)IR_TABLE_STEP)>>(IR_TABLE_RESOLUTION - IF_RESOLUTION);
        }
        constexpr sim2uint_t IDtoIR(uint32 id){
            return ((sim2uint_t)id)*IR_TABLE_STEP;
        }
        constexpr sim2uint_t ID_FRACtoIR(uint32 id,uint32 frac){
            return ((sim2uint_t)id)*IR_TABLE_STEP
            + (sim2uint_t)(frac<<(IR_TABLE_RESOLUTION - IF_RESOLUTION));
        }
        
        sim2int_t ID_FRACtoX(uint32 id, uint32 frac);
        sim2int_t ID_FRACtoY(uint32 id, uint32 frac);
        simint_t ID_FRACtoCOS(uint32 id, uint32 frac);
        simint_t ID_FRACtoSIN(uint32 id, uint32 frac);
        
        //V -> R Conversion
        constexpr int IV_IR_TABLE_RESOLUTION = 14;
        constexpr int IV_RESOLUTION_OF_IV_IR_TABLE = IV_RESOLUTION - IV_IR_TABLE_RESOLUTION;
        constexpr simint_t IV_IR_TABLE_STEP = static_cast<simint_t>(1)<<IV_RESOLUTION_OF_IV_IR_TABLE;//IVの幅
        
        sim2int_t IVtoIR(simint_t);
        
        //R -> V Conversion
        constexpr int IR_IV_TABLE_RESOLUTION = 14;
        std::array<simint_t, (1 << IR_IV_TABLE_RESOLUTION) + 4> IR_IVTable;
        constexpr int IR_RESOLUTION_OF_IR_IV_TABLE = IR_RESOLUTION - IR_IV_TABLE_RESOLUTION;//45 - 14
        constexpr sim2int_t IR_IV_TABLE_STEP = static_cast<sim2int_t>(1)<<IR_RESOLUTION_OF_IR_IV_TABLE;//IRの幅
        constexpr simint_t I32R_IV_TABLE_STEP = static_cast<simint_t>(IR_IV_TABLE_STEP >> 24);//IRの幅(simint_t演算にて)
        
        simint_t IRtoIV(sim2int_t);
        ...
        struct IMovingStone : public IMoment{
            ...
            sim2uint_t r;//distance from current position with no collision
        ...
        struct ISBoard{
            ISMovingStone awakeStone[16];//state of moving stones
            ...
            //result
            BitSet32 changed;//stones whose position were changed
            BitSet32 removed;//stones who were removed Play-Area
            BitSet32 inserted;//stones who were inserted Play-Area
            ...
            void set(const board_t& bd){
                NAsleep = 0;
                iterateAllStonesWithState(bd, [this](uint32 idx, const auto& st)->void{
                    asleepStone[NAsleep].set(FRtoI64R(st.getX()), FRtoI64R(st.getY()), st.state);
                    asleep.set(idx);
                    NAsleep++;
                }
                                          }
