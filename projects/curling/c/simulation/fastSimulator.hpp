...
namespace DigitalCurling{
    namespace FastSimulator{
        ...
        void FastSimulatorBoard::setAfterFirstColl(const board_t& bd, uint32 awNum, uint32 asCollidedNum){
            // 最初の衝突後にデータをセット
            turn = awNum;
            ...
            asleep = 0U;
            int cnt = NAwake;
            iterateStoneWithIndexWithoutFirstCheckExcept(bd, asCollidedNum, [this, &bd, asCollidedNum, &cnt](uint32 idx, const auto& st)->void{
                asleep.set(cnt);
            ...
            //停止位置までの距離を入れる
            //stone(0).r_goal = FVtoFR(stone(0).v);
            //stone(1).r_goal = FVtoFR(stone(1).v);
        ...
        bool simulateF_b2d_FSB_inContact(fsbd_t *const pbd, board_t *const pabd, const fpn_t step){
            ...
            // awakeだった石のデータを更新
            iterate(pbd->awake, [&b2bd, &newAwake, &newAsleep, &removed, pbd, pabd, newTime, &foul](uint32 idx)->void{
        ...
        ContactTree simulateSoloF(board_t *const pbd, uint32 stNum, const move_t& mv){
            // ただ停止位置を計算し、石を置くだけ
        ...
        ContactTree loop1_1(board_t *const pbd, uint32 awNum, mst_t& aw){
            ...
            for (;;){
                ...
                if (r2_asleep < pow(FR_COLLISION + 2 * FR_STONE_RAD, 2)){ // 衝突
                    ...
                    fpn_t nr = sqrt(aw.gr * aw.gr + r_asleep * r_asleep - 2.0 * aw.gr * r_asleep * cos(FTHETA_CURL_MAX));
        ...
        ContactTree loopAfterFirstCollision(board_t *const pabd, fsbd_t *const pbd){
            ...
            SOLVE_CONTACT:
                if(NContacts.any()){ // コンタクトあり
                    if(contactPhase || (NContacts & (~0x1111111111111111ULL))){
                        ...
                        iterateAnyWithoutFirstCheck(contactArray, [&](uint32 value)->void{
                            if(!(value & (1 << 8))){ // aw_as
                                uint32 iaw = value & 15;
                                uint32 ias = (value >> 4) & 15;
                                auto& aw = pbd->stone(iaw);
                                auto& as = pbd->stone(ias);
        ...
        template<int RINK_ONLY = 1, class board_t, class mst_t>
        ContactTree loop1_N(board_t *const pbd, uint32 awNum, mst_t& aw){
        ...
        ContactTree simulateF(board_t *const pbd, uint32 stNum, const move_t& mv){
            ...
            if (countNInPlayArea(*pbd) == 0){
                ret = simulateSoloF<RINK_ONLY>(pbd, stNum, mv);
            }else{
                FSMovingStone st;
                st.set(FX_THROW, FY_THROW, min(FV_SIMU_MAX, (fpn_t)mv.v()), mv.th(), SpintoThrowFW(mv.getSpin()), 0);
                
                //cerr << SpintoThrowFW(mv.getSpin()) << endl;
                
                if(countNInPlayArea(*pbd) == 1){
                    ret = loop1_1<RINK_ONLY>(pbd, stNum, st);
                }else{
                    ret = loop1_N<RINK_ONLY>(pbd, stNum, st);
                }
            }
            ...
            if(RINK_ONLY){
                ...
                //removeStones(pbd, [](const auto& st)->bool{ return !isInPlayArea(st); });
            }
            return ret;
        }
        /*
        template<class board_t, class pos_t>
        int simulateDrawF(board_t *const pbd, const pos_t& pos, int spin)
        {
        // 高速シミュレータ(停止位置を受ける)
        int ret;

        if (pos.y < pbd->frontY - 2*FR_STONE_RAD ){
        // どの石にも届かず停止する
        return simulateSoloDrawF(pbd, pos);
        }
        else{
        fpn_t w = (spin!=SPIN_RIGHT) ? (-FANGV_ORG) : FANGV_ORG;
        if (countNInPlayArea(*pbd) == 1){
        //tick();
        ret = loopHitZone1_1<1>(pbd, fPosXY<>(FX_THROW, FY_THROW), w, v_goal, v_theta, r_goal);
        //tock();
        }
        else{
        //tick();
        ret = loopHitZone1_N<1>(pbd, fPosXY<>(FX_THROW, FY_THROW), w, v_goal, v_theta, r_goal);
        //tock();
        }
        }
        return ret;
    ...
    ContactTree makeMoveNoRand(board_t *const pbd, uint32 idx, const move_t& mv){
        // 石を投げて、盤面を更新する(連続直交ベクトル)
        // 外乱なし
        
        // (物理演算的に)非合法な手を合法な手に変える処理
        /*fMoveXY<> tmp;
        tmp.s = mv.getSpin();
        fpn_t v2 = mv.v2();
        if (v2 >= FV2_MAX){
    ...
    ContactTree makeMove(board_t *const pbd, uint32 stNum, const move_t& mv, dice_t *const pdice){
        // 石を投げて、盤面を更新する(連続直交ベクトル)
        // 外乱あり
