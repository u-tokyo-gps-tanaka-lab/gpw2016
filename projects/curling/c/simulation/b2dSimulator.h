...
namespace DigitalCurling{
    namespace b2dSimulator{
        ...
        struct b2BoardStone : public b2IdentityBoardStone{
            uint32 index; // 詰めるために石番のデータも必要
            
            void set(uint32 ai, b2Body* ap){
        ...
        struct b2Board{
            ...
            void setAwakeStone(uint32 num, const mst_t& org){
                // 動く石を1つセット
            ...
            void setAsleepStones(const board_t& bd){
                ...
                int cnt = NAwake;
                iterateStoneWithIndex(bd, [&cnt, this](uint32 index, const auto& st)->void{
                    if (RINK_ONLY){
            ...
            void setAsleepStone(uint32 stNum, const st_t& org){
                // 静止している石を1つセット
                // 動く石のセットの後の必要あり
        ...
        int simulateb2d(board_t *const, uint32, const mst_t&, const fpn_t);
        
        template<int RINK_ONLY = 1, class board_t, class move_t>
        int simulate(board_t *const, uint32, const move_t& mv);
    }
}
