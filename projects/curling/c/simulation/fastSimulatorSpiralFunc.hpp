...
namespace DigitalCurling{
    namespace FastSimulator{
        ...
        void FVThtoFRTh(fpn_t *const pfr, fpn_t *const pfth, fpn_t fv, fpn_t fth, int s){
            *pfr = FVtoFR(fv);
        ...
        void FXYVThtoFXYRTh(fpn_t *const pfx, fpn_t *const pfy, fpn_t *const pfr, fpn_t *const pfth,
            fpn_t fx, fpn_t fy, fpn_t fv, fpn_t fth, int s){
            FVThtoFRTh(pfr, pfth, fv, fth, s);
            *pfx = fx + *pfr * sin(*pfth);
            *pfy = fy + *pfr * cos(*pfth);
        }
        ...
        fpn_t FVtoFX(fpn_t fv){
            ...
            //return (FV_FAllTable[id].x * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].x * frac) * FV_TABLE_INVSTEP;
            return -FDX_FV_FR * fv * fv;
        }
        fpn_t FVtoFY(fpn_t fv){
            //uint32 id = FVtoID(fv); fpn_t frac = FVtoFRAC(fv);
            //return (FV_FAllTable[id].y * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].y * frac) * FV_TABLE_INVSTEP;
            return -FDY_FV_FR * fv * fv;
        }
        fpn_t FVtoFTh(fpn_t fv){
            //uint32 id = FVtoID(fv); fpn_t frac = FVtoFRAC(fv);
            //return (FV_FAllTable[id].th * (FV_TABLE_STEP - frac) + FV_FAllTable[id + 1].th * frac) * FV_TABLE_INVSTEP;
            
            return -((log(FVtoFR(fv) / FA_CURVE) / FB_CURVE) + FTHETA_CURL);
            
            //return log(fv * fv) * atan2(FVtoFY())
        }
        ...
        void FV_FRtoFXY(fpn_t v, fpn_t r, pos_t *const pos, fpn_t *const theta, int spin){
            // 初速vのショットが投擲点から距離rの点での相対位置を求める
            // もしrまで到達しないならば、適当な値が帰るので注意
            // ヒット系のショットの生成の基本関数
            assert(r >= 0 && v >= 0);
            const fpn_t fv = min(v, FV_MAX);
            //const fpn_t r2 = r*r;
            int id = FVtoID(fv);
            fpn_t gx, gy, x, y;
            if (isRight(spin)){
