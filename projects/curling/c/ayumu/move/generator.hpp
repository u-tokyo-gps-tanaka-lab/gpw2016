...
namespace DigitalCurling{
    ...
    /*
     ...
     //1石相対
     iterateStoneWithIndex(bd, [&](uint32 idx, const auto& st)->void{
     ...
     //2石相対
     iterateStoneWithIndex(bd, oppColor, [&, oppColor](uint32 idx, const auto& st)->void{
     iterate(st.tailStone(), [&, oppColor](uint32 idx1)->void{
     ...
     //1石全体
     iterateStoneWithIndex(bd, oppColor, [&](uint32 idx, const auto& st)->void{
    ...
    int genSimpleVMove(move_t *const mv0, const board_t& bd){
        ...
        if(countNInHouse(bd, oppColor) > 0){ // ハウス内に相手の石がある
            const uint32 no1 = bd.centerInColor[oppColor][0]; // 相手の最も内側の石
        ...
        if (bd.isAlmostNull()){ // ほぼ空場の時は空場専用ルーチン
            ...
            if(sc != 0){ // ハウス内に石がある
                ...
                if (getColor(no1) == oppColor){ // 相手の石がNo1
                    ...
                    // ドローレイズ出来そうな自分の石を探す
                    iterateStoneWithIndex(bd, myColor,[&mv](uint32 i, const auto& st)->void{
                    ...
                    if(bd.NInHouse(oppColor)){
                        uint32 oppNo1 = bd.centerInColor[oppColor][0];
                ...
                if(clean){
                    ...
                    if(!isFreeGuardTurn(t)){
                        // ガードを外す
                        iterateStoneWithIndex(bd, oppColor, [&mv](uint32 i, const auto& st)->void{
                            if(isInGuardZone(st)){
                    ...
                    if(isBlack(myColor)){
                        ...
                        // ガードがあったらカムアラする
                        iterateStoneWithIndex(bd, [&mv, myColor](uint32 i, const auto& st)->void{
                            if(isInGuardZone(st)){
