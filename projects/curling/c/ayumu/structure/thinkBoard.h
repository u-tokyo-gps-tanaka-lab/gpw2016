...
namespace DigitalCurling{
    ...
    public:
        ...
        // 石へのアクセス
        const stone_t& stone(uint32 idx)const{ assert(examIndex(idx)); return stone_[idx]; }
        stone_t& stone(uint32 idx){ assert(examIndex(idx)); return stone_[idx]; }
        ...
        // 相対石情報
        const relStone_t& relStone(uint32 i0, uint32 i1)const{ return relStone_[i0][i1]; }
        relStone_t& relStone(uint32 i0, uint32 i1){ return relStone_[i0][i1]; }
        ...
        void updateInfoLast();
        void updateDiffInfo_removed(uint32 idx);
        void updateDiffInfo_relocated(uint32 idx);
        void updateDiffInfo_new(uint32 idx);
        ...
        template<class pos_t>
        FORCE_INLINE void locateStone(uint32 idx, const pos_t& pos){
        ...
        template<class pos_t>
        FORCE_INLINE void locateNewStone(uint32 idx, const pos_t& pos){
            Color c = getColor(idx);
        ...
        template<class pos_t>
        FORCE_INLINE void relocateStone(uint32 idx, const pos_t& pos){
            ASSERT(sb().test(idx), cerr << "index = " << idx << endl;);
            Color c = getColor(idx);
            if(isInHouse(stone(idx))){
                --NInHouse_[c];
        ...
        FORCE_INLINE void removeStone(uint32 idx){
            ASSERT(sb().test(idx), cerr << "index = " << idx << endl;);
            Color c = getColor(idx);
            if(isInHouse(stone(idx))){
                --NInHouse_[c];
    ...
    private:
        ...
        void iterateStone(const callback_t& callback)noexcept{
            iterate(sb(), [this, callback](uint32 idx)->void{
                callback(stone_[idx]);
            });
        }
        template<typename callback_t>
        void iterateStone(Color c, const callback_t& callback){
            iterate(sbColor(c), [this, callback](uint32 idx)->void{
                callback(stone_[idx]);
            });
        }
        template<typename callback_t>
        ...
        void insertStoneSequence(uint32 idx, seq_t& seq, int i, callback_t callback);
    ...
    public:
        ...
        uint64 hash_stones[HASH_RANGE]; // 局面認識ハッシュ値の石成分(レンジ幅分)
        uint64 hash_orderBits; // 局面認識ハッシュ値のオーダー成分
    ...
    ContactTree makeMoveNoRand(ThinkBoard *const, uint32, const move_t&);
    template<int RINK_ONLY = 1, class move_t>
    ContactTree makeMoveNoRand(ThinkBoard *const, const move_t&);
    
    template<int RINK_ONLY = 1, class dice_t>
    ContactTree makeMove(ThinkBoard *const, uint32, const fMoveXY<>& mv, dice_t *const);
    template<int RINK_ONLY = 1, class dice_t>
    ...
    void copyBeforePlayout(const ThinkBoard& arg, ThinkBoard *const dst)noexcept{
        ...
        iterateStoneWithIndex(arg, [&arg, dst](uint32 idx, const auto& st)->void{
            dst->stone_[idx] = st;
            iterateExcept(arg.sb(), idx, [&arg, dst, idx](uint32 idx1)->void{
