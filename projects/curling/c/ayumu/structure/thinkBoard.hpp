...
namespace DigitalCurling{
    ...
    void ThinkBoard::insertStoneSequence(uint32 idx, seq_t& seq, int i, callback_t callback){
        for (int j = 0; j < i; ++j){
    ...
    void ThinkBoard::updateBaseRange(){
        ...
        iterateStoneWithIndex([this](int i0, auto& st0)->void{
            ...
            st0.baseRange = min(8U, bsr((uint32)max(1, ((int)score))));
    ...
    */
    ...
    void ThinkBoard::updateInfo(int mode){
        ...
        if (NInPlayArea() != 0){
            ...
            iterateStoneWithIndex([this](uint32 idx, auto& si)->void{
                ...
                iterateExcept(this->sb(), idx, [this, idx, &si](uint32 idx1)->void{
            ...
            // 石順序の作成
            {
                ...
                for (int c = 0; c < 2; ++c){
                    int cnt = 0;
                    iterate(sbColor(static_cast<Color>(c)), [this, c, &cnt](uint32 idx)->void{
    ...
    void ThinkBoard::updateDiffInfo_removed(uint32 idx){
        // 石列処理
        //Color c = getColor(idx);
        ...
        if (turn >= TURN_FREEGUARD - 1){
            ...
            if (turn == TURN_FREEGUARD - 1){
                ...
                iterateStone(oc, [](auto& st)->void{
                    st.state |= ((uint32)st.state & (1U << 2)) << 1;
                });
            }
        }
        // それ以外の操作は必要なし
        ...
        if (turn >= TURN_FREEGUARD - 1){
            if (turn == TURN_FREEGUARD - 1){
                ...
                iterateStone(oc, [](auto& tsi)->void{
                    tsi.state |= ((uint32)tsi.state & (1U << 2)) << 1;
                });
    ...
    bool ThinkBoard::exam_stone()const{ // 石の情報
        if (searchStoneWithIndex(*this, [](uint32 idx, const auto& si)->bool{
    ...
    bool ThinkBoard::exam_hash()const{
        ...
        if (NInPlayArea() > 0){
            for (int r = 0; r < HASH_RANGE; ++r){
                uint64 hash_tmp = 0ULL;
                DigitalCurling::iterateStoneWithIndex(*this, [&hash_tmp, r](uint32 idx, const auto& st)->void{
#if defined(USE_BASE_RANGE)
    ...
    bool ThinkBoard::exam_stone_turn(int t)const{
        ...
        if (searchStoneWithIndex(*this, [t](uint32 idx, const auto& si)->bool{
            if (idx <= t){
    ...
    std::string ThinkBoard::toString()const{
        ...
        oss << "B";
        DigitalCurling::iterateStoneWithIndex(*this, BLACK, [&oss](uint32 idx, const auto& si)->void{
            oss << ' ' << idx << si.toString();
        });
        oss << "W";
        DigitalCurling::iterateStoneWithIndex(*this, WHITE, [&oss](uint32 idx, const auto& si)->void{
            oss << ' ' << idx << si.toString();
        });
        return oss.str();
    }
    ...
    ContactTree makeMoveNoRand(ThinkBoard *const pbd, uint32 idx, const move_t& mv){
        // 非合法な手を合法な手に変える
        /*fMoveXY<> tmp;
        tmp.s = mv.getSpin();
        fpn_t v2 = mv.v2();
    ...
    ContactTree makeMove(ThinkBoard *const pbd, uint32 stNum, const fMoveXY<>& mv, dice_t *const pdice){
