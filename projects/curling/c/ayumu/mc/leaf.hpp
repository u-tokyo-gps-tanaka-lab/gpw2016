    ...
    namespace Ayumu{
                ...
                do{
                    ...
                    if (bd.isAlmostNull()){
                        ...
                        for (int r = RANGE_MAX_LEAFNODE; r >= RANGE_MIN_LEAFNODE; --r){
                            uint64 hashValue = bd.getHash(r);
                            uint64 identityValue = decltype(bdTT)::page_t::knitIdentityValue(hashValue, bd.getTurn(), r);
                            bool found;
        ...
        void diveBoardTree(PlayoutResult *const presult, board_t& bd, node_t& node, const int range,
            ...
            if(isExpansionCondition(node, range)){ // さらに深く潜るとき
                ...
                uint64 hashValue = bd.getHash(nextRange);
                uint64 identityValue = decltype(bdTT)::page_t::knitIdentityValue(hashValue, bd.getTurn(), nextRange);
                bool found;
