...
namespace DigitalCurling{
    namespace Ayumu{
        ...
        struct RootGridChild : public Integrated2DGridLayer<RootGrid, W, L, ZW, ZL>{
            ...
            void set(const vmove_t& avmv, const move_t& amv, uint64 ahash){
                vMove = avmv;
                baseMove = amv;
                hash = ahash;
                size_ = 0;
        ...
        struct RootNode{
            ...
            std::atomic<uint32> trials;
            
            uint64 limitTime; // 時間制限
            uint64 limitPlayouts; // プレイアウト回数制限
            ...
            void addChild(const board_t& bd, const vmove_t& avmv, const move_t amv){
                uint64 hash_bd = bd.getHash(0);
                uint64 hash_move = genHash_AbsoluteMove(avmv);
                /*if(!avmv.hasContact()){
            ...
            void setLimitTime(uint64 alt)noexcept{
                limitTime = alt;
            }
            void setLimitPlayouts(uint64 alp)noexcept{
                limitPlayouts = alp;
            }
            /*
            ...
            int getBestUCBIndex(dice_t *const dice)const{
                ...
                iterateChild([&](int c, const auto& ch)->void{
                    ...
                    if (ch.size() < MIN_TRIALS_ROOT){
                        ...
                        //tmpScore = (fpn_t)((1U << 16) - ((uint32)ch.size()) + (dice->rand() % (1U << 6)));
                        tmpScore = -9;
                    }
                    else{
        ...
        void L1Sub(const int tid, Spin s, const ThinkBoard *const pbd){
            ...
            for(int w = tid; w <= ROOT_LAST_GRID_W; w += N_THREADS){ // 2016/8/26 田中先生がバグ発見
                for(int l = 0; l <= ROOT_LAST_GRID_L; ++l){
                    ...
                    MiniBoard mbd;
                    iterateStoneWithIndex(*pbd, [&mbd](uint32 idx, const auto& st){
        ...
        std::tuple<fMoveXY<>, eval_t, int> L1(const ThinkBoard& bd){
            ...
            for(int s = 0; s < 2; ++s){
                ...
                /*for(int w=0;w<ROOT_LAST_GRID_W;++w){
                 ...
                 MiniBoard mbd;
                 iterateStoneWithIndex(bd, [&mbd](uint32 idx, const auto& st){
                 mbd.locateNewStone(idx, st);
                 });
                 ContactTree ct=makeMoveNoRand<1>(&mbd, TURN_LAST, fmv);
