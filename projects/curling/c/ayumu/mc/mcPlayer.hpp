...
namespace DigitalCurling{
    namespace Ayumu{
        ...
        public:
            ...
            std::array<uint64, 2> decideLimits(const board_t& bd);
        ...
        template<class board_t>
        std::array<uint64, 2> MonteCarloPlayer::decideLimits(const board_t& bd){
            // 自分の着手決定限度時間、プレイアウト回数を設定
            
            constexpr uint64 byoyomiMargin = 100;
            ...
            if (myTimeLimit.isUnlimited()){ // 持ち時間無制限
                ...
                const uint64 insurance = 4000;
                const uint64 insuranceByShot = 500;
                //const uint32 shots_remained = getGameRemStones(bd.getTurnColor(), bd.getEnd(), bd.getTurn());
                ...
                const uint64 byoyomi = (myTimeLimit.byoyomi_ms > byoyomiMargin) ?
        ...
        int MonteCarloPlayer::play(fMoveXY<fpn_t> *const pdst, const board_t& bd, fpn_t *const pexpWP){
            ...
            if(bd.getTurn() == TURN_LAST){ // ラストストーン(L1)専用の解法
                ...
                *pdst = std::get<0>(r);
                ...
                /*if(tb.getTurn() == TURN_LAST){
                 MiniBoard mbd;
                 iterateStoneWithIndex(tb, [&mbd](uint32 idx, const auto& st){
                ...
                //tickMicS();
                if(bdTT.end() < INT_MAX){
                    if(bd.isNull() || bdTT.end() != bd.getEnd()){
                        bdTT.init();
                    }else{
                        //cerr << "proc" << endl;
            ...
            // じりつくん法
            auto best = Jiritsu::doMonteCarloSearch(tb, settledPlayouts, [&](const auto& bd, const auto& move)->eval_t{
                ThinkBoard tbd = bd;
