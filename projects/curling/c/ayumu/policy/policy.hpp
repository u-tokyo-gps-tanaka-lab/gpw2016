...
#undef FooXC
    ...
    void playWithBestPolicy(move_t *const pmv, const board_t& bd, const policy_t& pol, tools_t *const ptools){
        ...
        if(NMoves != 1){
            ...
            std::stable_sort(buf.begin(), buf.begin() + NMoves, std::greater<fpn_t>());
            for(int i = 0; i < NMoves; ++i){
                cerr << i << buf[i] << exp(buf[i].weight / pol.temperature()) << endl;
            }
