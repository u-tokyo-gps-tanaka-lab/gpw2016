...
namespace DigitalCurling{
    ...
    struct ClientState{
        std::atomic<uint32> st;
        
        uint32 isPondering()const{ return st & 2U; }
        uint32 isDecidingMove()const{ return st & 1U; }
        uint32 requestedToStop()const{ return st & 4U; }
        uint32 requestedToExit()const{ return st & 32U; }
        uint32 requestedToPonder()const{ return st & 16U; }
        uint32 requestedToDecideMove()const{ return st & 8U; }
        
        uint32 isThinking()const{ return st & 3U; }
...
namespace DigitalCurling{
    ...
    public:
        ...
        virtual void setRandomSeed(uint64 s)noexcept{
            // 乱数シード固定(デバッグ用)
            dice.srand(s);
            ddice.srand((uint32)dice.rand());
        ...
        virtual int initAll(){
            ...
            // 各スレッドの持ち物を初期化
            setRandomSeed((uint32)time(NULL));
