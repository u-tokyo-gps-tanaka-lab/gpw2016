...
namespace DigitalCurling{
    struct FrontTakeOut : public ShotGenerator{
        ...
        static uint32 FRtoID(fpn_t fr){
            return (uint32)((int)((fr - FR_MIN)*((RESOLUTION_R-1)*RESOLUTION_HASU)) / (FR_MAX - FR_MIN),0);
        }
        static uint32 FThtoID(fpn_t fth){
            return (uint32)((fth - FTH_MIN)*((RESOLUTION_TH-1)*RESOLUTION_HASU) / (FTH_MAX - FTH_MIN));
        }
