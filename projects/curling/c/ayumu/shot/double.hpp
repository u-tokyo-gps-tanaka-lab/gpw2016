...
namespace DigitalCurling{
    ...
    struct Double : public ShotGenerator{
        ...
        constexpr static int64 RESOLUTION_R = 32;
        constexpr static int64 RESOLUTION_TH = 32;
        
        constexpr static int64 RESOLUTION_HASU = 1LL << 6;
        ...
        static int64 FRtoID(fpn_t fr){
            return max(int64((fr - FR_MIN) * ((RESOLUTION_R - 1) * RESOLUTION_HASU) / (FR_MAX - FR_MIN)), 0LL);
        }
        static int64 FThtoID(fpn_t fth){
            return max(int64((fth - FTH_MIN) * ((RESOLUTION_TH - 1) * RESOLUTION_HASU) / (FTH_MAX - FTH_MIN)), 0LL);
        }
        
        template<class board_t, class vmove_t>
        double estimate(const board_t& bd, uint32 stNum, const vmove_t& vmv){
            uint32 obj0 = vmv.getNum0(), obj1 = vmv.getNum1();
            return 0.0;
        }
        
        template<class board0_t, class board1_t, class vmove_t>
        double evaluate(const board0_t& pre, const board1_t& post, uint32 stNum, const vmove_t& vmv, const ContactTree& ct = 0)const{
            // テイクアウトしたい石のそれぞれに対して、
            // アクティブゾーン外に行った... + 0.5
            // ハウスからは出た... + 0.45
            const uint32 obj0 = vmv.getNum0(), obj1 = vmv.getNum1(); // テイクアウトしたい石
    ...
    double Double::genReal2(move_t *const pmv, const pos_t& front, const pos_t& back){
        ...
        if (r < FR_MAX){
            ...
            int64 ir = FRtoID(r);
            int64 ith = FThtoID(th);
            
            //cerr << "ir = " << ir << " ith = " << ith << endl;
            
            int64 idBase = ((ir / RESOLUTION_HASU) * RESOLUTION_TH + (ith / RESOLUTION_HASU)) * 2 + 0;
            int64 hasu[2] = { ir % RESOLUTION_HASU, ith % RESOLUTION_HASU, };
