...
namespace DigitalCurling{
    ...
    constexpr uint32 I16R_IN_HOUSE = (1U << 12);
    
    constexpr uint32 I16X_MID = 1U << (RESOLUTION_I16X - 1);
    constexpr uint32 I16Y_MID = 1U << (RESOLUTION_I16Y - 1);
    
    constexpr int RESOLUTION_I16TH = 15;
    constexpr uint32 I16TH_MIN = 0;
    constexpr uint32 I16TH_MAX = (1U << RESOLUTION_I16TH) - 1U;
    ...
    constexpr fpn_t I16RtoFR(uint32 r)noexcept{
        return (fpn_t)r * FR_I16R_STEP;
    }
    constexpr fpn_t I16XtoFX(uint32 x)noexcept{
        return (fpn_t)x * FR_I16R_STEP + FX_I16X0;
    }
    constexpr fpn_t I16YtoFY(uint32 y)noexcept{
        return (fpn_t)y * FR_I16R_STEP + FY_I16Y0;
    }
    
    constexpr uint32 FRtoI16R(fpn_t f)noexcept{
        return (uint32)(f * FR_I16R_INVSTEP);
    }
    constexpr uint32 FXtoI16X(fpn_t fx)noexcept{
        return uint32(int32(I16X_MID) + int32((fx - FX_XMID) * FR_I16R_INVSTEP));
    }
    constexpr uint32 FYtoI16Y(fpn_t fy)noexcept{
        return uint32(int32(I16Y_MID) + int32((fy - FY_YMID) * FR_I16R_INVSTEP));
    }
    
    constexpr uint32 FR2toI16R2(float f2)noexcept{
        return (uint32)(f2 * static_cast<float>(FR_I16R_INVSTEP * FR_I16R_INVSTEP));
    }
    constexpr uint32 FR2toI16R2(double f2)noexcept{
        return (uint32)(f2 * static_cast<double>(FR_I16R_INVSTEP * FR_I16R_INVSTEP));
    }
    constexpr uint32 FR2toI16R2(long double f2)noexcept{
        return (uint32)(f2 * static_cast<long double>(FR_I16R_INVSTEP * FR_I16R_INVSTEP));
    }
    constexpr uint32 FTtoI16T(fpn_t ft)noexcept{
        return I16TH_MIN + (uint32)((ft + M_PI) * FTH_I16TH_INVSTEP);
    }
    
    /*************************16ビット離散座標でのルール**************************/
    
    constexpr uint32 I16X_TEE = FXtoI16X(FX_TEE);
    constexpr uint32 I16Y_TEE = FYtoI16Y(FY_TEE);
    
    // リンク内
    constexpr bool isOnRink(uint32 x, uint32 y)noexcept{
    ...
    constexpr bool isInPlayArea(uint32 x, uint32 y)noexcept{
    ...
    constexpr uint32 STONE_MASK_X = (1U << 16) - 1U;
    constexpr uint64 STONE_MASK_COLOR = 1ULL << 32;
    ...
    struct StoneXY{
        ...
        constexpr StoneXY(uint64 ast) : st(ast){}
        constexpr StoneXY(const StoneXY& ast) : st(ast.st){}
        
        constexpr operator uint64()const noexcept{ return st; }
        
        // 成分取り出し
        constexpr uint32 getX()const noexcept{ return ((uint32)st) & STONE_MASK_X; }
        constexpr uint32 getY()const noexcept{ return (((uint32)st) >> 16) & STONE_MASK_X; }
        constexpr uint32 getXY()const noexcept{ return (uint32)st; }
        constexpr uint32 getCol()const noexcept{ return (uint32)(st >> 32); }
        
        // 性質
        constexpr uint64 isWhite()const noexcept{ return st & STONE_MASK_COLOR; }
        ...
        /*template<int N>StoneXY div(int n); // N分割のn番目を計算する
         ...
         uint32 r = 1U << getRange();
         uint32 half_r = r >> 1; // 範囲の半分
    ...
    constexpr uint32 I16VX_MIN = 0;
    constexpr uint32 I16VY_MIN = 0;
    constexpr uint32 I16VX_MAX = (1U << RESOLUTION_I16VX) - 1U;
    constexpr uint32 I16VY_MAX = (1U << RESOLUTION_I16VY) - 1U;
    constexpr uint32 I16VX_MID = 1U << (RESOLUTION_I16VX - 1);
    constexpr uint32 I16VY_MID = 1U << (RESOLUTION_I16VY - 1);
    
    //constexpr uint32 V_RANGE_MAX = (1U << RESOLUTION_VRANGE);
    ...
    constexpr uint32 MOVE_MASK_X = (1U << 16) - 1U;
    constexpr uint32 MOVE_MASK_Y = MOVE_MASK_X << MOVE_LCT_Y;
    constexpr uint64 MOVE_MASK_RANGE = 15ULL << MOVE_LCT_RANGE;
    constexpr uint64 MOVE_MASK_SPIN = 1ULL << MOVE_LCT_SPIN;
    constexpr uint64 MOVE_MASK_RELATIVITY = 1ULL << MOVE_LCT_RELATIVITY;
    constexpr uint64 MOVE_MASK_CONTACT = 1ULL << MOVE_LCT_CONTACT;
    constexpr uint64 MOVE_MASK_NUM0 = 15ULL << MOVE_LCT_NUM0;
    constexpr uint64 MOVE_MASK_NUM1 = 15ULL << MOVE_LCT_NUM1;
    constexpr uint64 MOVE_MASK_NUM2 = 255ULL << MOVE_LCT_NUM2;
    constexpr uint64 MOVE_MASK_TYPE = 255ULL << MOVE_LCT_TYPE;
    
    constexpr uint64 MOVE_PASS = MOVE_MASK_RELATIVITY;
    constexpr uint64 MOVE_ILLEGAL = uint64(-1);
    ...
    struct MoveXY{
        ...
        constexpr MoveXY() :mv(){}
        constexpr MoveXY(uint64 amv) : mv(amv){}
        constexpr MoveXY(const MoveXY& amv) : mv(amv.mv){}
        
        constexpr operator uint64()const noexcept{ return mv; }
        
        constexpr uint64 data()const noexcept{ return mv; }
        uint64& data()noexcept{ return mv; }
        
        void setNR(uint32 ax, uint32 ay, uint64 s)noexcept{ // ノーレンジ着手をセット
            mv = ax | (ay << MOVE_LCT_Y) | (s << MOVE_LCT_SPIN);
        }
        void clear()noexcept{ mv = 0ULL; }
        
        // 成分取り出し
        constexpr uint32 vx()const noexcept{ return (((uint32)mv) >> MOVE_LCT_X) & (MOVE_MASK_X >> MOVE_LCT_X); }
        constexpr uint32 vy()const noexcept{ return (((uint32)mv) >> MOVE_LCT_Y) & (MOVE_MASK_Y >> MOVE_LCT_Y); }
        constexpr uint32 vxy()const noexcept{ return (uint32)mv; }
        constexpr Spin getSpin()const noexcept{ return static_cast<Spin>(((uint32)(mv >> MOVE_LCT_SPIN)) & 1U); }
        constexpr Spin spin()const noexcept{ return getSpin(); }
        constexpr uint32 getRange()const noexcept{ return ((uint32)(mv >> MOVE_LCT_RANGE)) & 15U; }
        constexpr uint32 getNum0()const noexcept{ return ((uint32)(mv >> MOVE_LCT_NUM0)) & 15U; }
        constexpr uint32 getNum1()const noexcept{ return ((uint32)(mv >> MOVE_LCT_NUM1)) & 15U; }
        constexpr uint32 getNum2()const noexcept{ return ((uint32)(mv >> MOVE_LCT_NUM2)) & 255U; }
        constexpr uint32 getType()const noexcept{ return ((uint32)(mv >> MOVE_LCT_TYPE)); }
        
        constexpr uint32 getPartVX()const noexcept{ return ((uint32)mv) & MOVE_MASK_X; }
        constexpr uint32 getPartVY()const noexcept{ return ((uint32)mv) & MOVE_MASK_Y; }
        
        constexpr uint64 anyRange()const noexcept{ return mv & MOVE_MASK_RANGE; }
        constexpr uint64 isLeftSpin()const noexcept{ return mv & MOVE_MASK_SPIN; }
        constexpr uint64 isRelative()const noexcept{ return mv & MOVE_MASK_RELATIVITY; }
        constexpr uint64 hasContact()const noexcept{ return mv & MOVE_MASK_CONTACT; }
        
        bool isPASS()const noexcept{ return (mv == MOVE_PASS); }
        bool isILLEGAL()const noexcept{ return (mv == MOVE_ILLEGAL); }
        
        void setSpin(uint64 s)noexcept{ mv |= (s << MOVE_LCT_SPIN); }
        void setRelativity()noexcept{ mv |= MOVE_MASK_RELATIVITY; }
        void setContact()noexcept{ mv |= MOVE_MASK_CONTACT; }
        void setType(uint64 at)noexcept{ mv |= (at << MOVE_LCT_TYPE); }
        void setNum0(uint64 an)noexcept{ mv |= (an << MOVE_LCT_NUM0); }
        void setNum1(uint64 an)noexcept{ mv |= (an << MOVE_LCT_NUM1); }
        void setNum2(uint64 an)noexcept{ mv |= (an << MOVE_LCT_NUM2); }
        
        void setWR(Spin as)noexcept{ // 全範囲着手
            mv = (I16VX_MID << MOVE_LCT_X)
            | (I16VY_MID << MOVE_LCT_Y)
            | (((uint64)as) << MOVE_LCT_SPIN)
            | ((uint64)(RESOLUTION_I16VRANGE) << MOVE_LCT_RANGE);
        }
        // pass
        ...
        MoveXY part(int n)const{
            ...
            switch (N){
                case 4:{
                    uint32 half_r = 1U << (getRange() - 2); // レンジ幅の1/4
                    switch (n){
        ...
        MoveXY genNR(dice_t *const dice)const{
            ...
            //uint32 range = 1U << (getRange() - 1U);
            //uint32 josu = (range << 1) + 1U;
            
            uint32 range = 1U << getRange();
            uint32 josu = range + 1;
            range >>= 1;
        ...
        void mean(MoveXY amv)noexcept{
            // X, Y をamvとの間で平均化する
            constexpr uint32 mask = (MOVE_MASK_X >> 1) | ((MOVE_MASK_X >> 1) << MOVE_LCT_Y);
            uint32 nxy = vxy() + amv.vxy();
            mv = (mv & (~(MOVE_MASK_X | MOVE_MASK_Y))) | ((nxy >> 1) & mask);
        }
        
        // short func
        uint32 n0()const noexcept{ return getNum0(); }
        uint32 n1()const noexcept{ return getNum1(); }
    };
    ...
    constexpr fpn_t I16VtoFV(uint32 i)noexcept{
        return ((fpn_t)i) * FV_I16V_STEP;
    }
    constexpr fpn_t I16VXtoFVX(uint32 x)noexcept{
        return ((fpn_t)x) * FV_I16V_STEP + FVX_MIN;
    }
    constexpr fpn_t I16VYtoFVY(uint32 y)noexcept{
        return ((fpn_t)y) * FV_I16V_STEP + FVY_MIN;
    }
    
    template<typename float_t>
    constexpr uint32 FVtoI16V(float_t f)noexcept{
        return (uint32)(f * FV_I16V_INVSTEP);
    }
    template<typename float_t>
    constexpr uint32 FVXtoI16VX(float_t fx)noexcept{
        return I16VX_MIN + (uint32)((fx - FVX_MIN) * FV_I16V_INVSTEP);
    }
    template<typename float_t>
    constexpr uint32 FVYtoI16VY(float_t fy)noexcept{
        return I16VY_MIN + (uint32)((fy - FVY_MIN) * FV_I16V_INVSTEP);
    }
    template<typename float_t>
    constexpr uint32 FV2toI16V2(float_t f2)noexcept{
        return (uint32)(f2 * static_cast<float_t>(FV_I16V_INVSTEP * FV_I16V_INVSTEP));
    }
    ...
    void convMove(const movexy_t& arg, MoveXY *const dst)noexcept{
        dst->mv = (uint64)((FVXtoI16VX(arg.vx()) << MOVE_LCT_X) | (FVYtoI16VY(arg.vy()) << MOVE_LCT_Y))
        | (((uint64)arg.getSpin()) << MOVE_LCT_SPIN);
    }
    ...
    void convRMove(const fRMoveXY<float_t>& arg, MoveXY *const dst)noexcept{
        dst->mv = (uint64)((FVXtoI16VX(arg.vx()) << MOVE_LCT_X) | (FVYtoI16VY(arg.vy()) << MOVE_LCT_Y))
        | ((uint64(bsr64((uint64)FVtoI16V(arg.getRangeVX())))) << MOVE_LCT_RANGE) | (((uint64)arg.getSpin()) << MOVE_LCT_SPIN);
    }
    ...
    OrderBits maskUpper(OrderBits o, int n){
        ...
        //cerr << "fp = " << fp << endl;
        uint32 b = NthLowestBit64(fp, n + 1);
        if(b){
            //cerr << "lbi = " << bsf(b) << endl;
            const uint32 color = (-((o >> (bsf(b) - 1)) & 1));
            const uint32 mask = b - 1; // iより下の部分のマスク
            //cerr << "clr = " << color << endl;
            //cerr << "msk = " << mask << endl;
    ...
    //uint64 hash_table_pos[2][2][16][2];
    uint64 hash_table_pos[N_STONES][2][16][2];
    
    constexpr bool examAB(int ab)noexcept{ return (ab == 0 || ab == 1); }
    
    // 中央からの石順序
    //uint64 hash_table_order[2][16];
    
    // 絶対着手は(vx,vy,回転)の3次元
    // それにvx,vyの範囲を含めた4次元のそれぞれにハッシュ値を与える
    // 回転は現在2値なので、ハッシュ値も0と1とする
    
    uint64 hash_table_v[2][16][2];
    
    //uint64 hash_table_s[2];
    std::array<uint64, 16> hash_table_vr;
    
    
    /**************************石の位置ハッシュ関数**************************/
    
    uint64 genHash_Pos(uint32 idx, int ab, uint32 val, int r = 0){
        ...
        if(r < 16){
            val >>= r;
            uint64 ret = hash_table_pos[idx][ab][r][val & 1U];
    ...
    /*void addHash_Pos(uint64 *const dst, uint32 idx, uint32 a, uint32 b, int rmin = 0, int rmax = 15, int rbase = 0){
        ...
        for (int i = rmin + rbase; i <= rmax; ++i){
            uint64 hd = hash_table_pos[idx][0][i][a & 1U] ^ hash_table_pos[idx][1][i][b & 1U];
            int j = rmin;
            do{
                dst[j] ^= hd;
            } while (j++ < i);
    ...
    uint64 genHash_OrderBits(OrderBits o)noexcept{
        //cerr << o << " -> ";
        OrderBits oo = o; //maskUpper(o, 3);
        //cerr << oo << endl; if(o != oo){ getchar(); }
        return (((uint64)(oo & (0x0FFFU))) << 17);
    }
    
    /**************************着手成分ハッシュ関数**************************/
    
    uint64 genHash_V(int ab, uint32 a){
        assert(examAB(ab));
        uint64 ret = hash_table_v[ab][0][a & 1U];
        for (int i = 1; i < 16; ++i){
            a >>= 1;
    ...
    uint64 genHash_VRange(uint32 r){
        return hash_table_vr[r];
    }
    constexpr uint32 genHash_S(uint32 s)noexcept{
        return (s << 16);
    }
        
    void subtrHash_VRange(uint64 *const dst, int org_range){
        //レンジを狭くする
        *dst ^= hash_table_vr[org_range];
        *dst ^= hash_table_vr[org_range - 1];
    }
    
    uint64 genHash_VAB(uint32 val){
        // 下半分がa、上半分がbとする形
        uint64 ret = hash_table_v[0][0][val & 1U];
        for (int i = 1; i < 16; ++i){
            val >>= 1;
    ...
    uint64 genHash_AbsoluteMove(const MoveXY mv){
    ...
    constexpr uint64 genHash_RelativeMove(const MoveXY mv)noexcept{
        return ((uint64)mv) & (MOVE_MASK_TYPE | MOVE_MASK_NUM0 | MOVE_MASK_NUM1 | MOVE_MASK_SPIN) & (~MOVE_MASK_RANGE);
    }
    
    void initHash(){
        // ハッシュ関連初期化
        
        const uint32 seed = 0x0e4a0fe9;
        XorShift64 dice;
