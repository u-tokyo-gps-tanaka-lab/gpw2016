...
namespace DigitalCurling{
    ...
    namespace Jiritsu{
        ...
        private:
            ...
            // 全体格子点の数
            static constexpr size_t W = static_cast<size_t>(abs(DX / GX) + 0.999999);
            static constexpr size_t L = static_cast<size_t>(abs(DY / GY) + 0.999999);
        ...
        std::tuple<fMoveXY<>, eval_t> doMonteCarloSearch(const board_t& bd, int playouts,
                                                         const playout_t& playout,
                                                         dice_t *const pdice){
            LayerNode node;
            node.init();
