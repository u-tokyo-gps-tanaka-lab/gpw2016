...
struct DSFMT{
    ...
    DSFMT(uint32 s) :dsfmt(){ dsfmt_init_gen_rand(&dsfmt, s); }
    DSFMT() :dsfmt(){}
};
...
namespace DigitalCurling{
    ...
    constexpr Color toTurnColor(uint32 t)noexcept{
    ...
    constexpr Color getColor(uint32 id)noexcept{
        return static_cast<Color>(id & 1);
    }
    constexpr uint32 getIndexInColor(uint32 id)noexcept{ return (id >> 1); }
    
    constexpr bool examIndex(uint32 id)noexcept{
        return (0 <= id && id < N_STONES);
    }
    constexpr bool examIndexInColor(uint32 id)noexcept{
        return (0 <= id && id < N_COLOR_STONES);
    }
    
    // 手番、石番から先手後手合わせた石番(色判別可能)への変換
    constexpr uint32 CItoTurn(Color c, uint32 idx)noexcept{
        return (idx << 1) | c;
    }
    
    // 石番から色が同じか調べる
    constexpr bool isSameColorIndex(uint32 id0, uint32 id1)noexcept{
        return !((id0 + id1) & 1);
    }
    ...
    constexpr uint64 TIME_LIMIT_UNLIMITED = 0xffffffffffffffff;

    struct TimeLimit{
        uint64 limit_ms;
        uint64 byoyomi_ms;

        void set(uint64 aleft, uint64 aby = 0ULL)noexcept{
            limit_ms = aleft;
            byoyomi_ms = aby;
        }
        void subtr(uint64 aused)noexcept{
            limit_ms -= aused;
        }
        void add(uint64 aadded)noexcept{
            limit_ms += aadded;
        }
    ...
    */
    ...
    // 1つを除き、それ以外を見る
    template<class board_t, typename callback_t>
    void iterateStoneWithIndexExcept(const board_t& bd, uint32 ex, const callback_t& callback){
        iterateExcept(bd.sb(), ex, [bd, callback](const int idx)->void{
            callback(idx, bd.stone(idx));
        });
    }
    template<class board_t, typename callback_t>
    void iterateStoneWithIndexExcept(const board_t& bd, Color c, uint32 ex, const callback_t& callback){
    ...
    template<class board_t, typename callback_t>
    void iterateStoneWithIndexWithoutFirstCheckExcept(const board_t& bd, uint32 ex, const callback_t& callback){
        iterateWithoutFirstCheckExcept(bd.sb(), ex, [bd, callback](const int idx)->void{
            callback(idx, bd.stone(idx));
        });
    }
    template<class board_t, typename callback_t>
    void iterateStoneWithIndexWithoutFirstCheckExcept(const board_t& bd, Color c, uint32 ex, const callback_t& callback){
    ...
    fpn_t calcDistanceSumOneToOne(const board0_t& bd0, const board1_t& bd1, int *const pdist = nullptr){
        ...
        double sum = 0;
        iterate(bd0.sb(), [bd0, bd1, pdist, &sum](uint32 idx)->void{
            double d = calcDistance(bd0.stone(idx), bd1.stone(idx));
            sum += d;
            if(pdist != nullptr){
                int l = int(d * 1000);
                pdist[(l < 1) ? 0 : (log2i(uint32(l)) + 1)]++;
            }
        });
        return sum;
    }
    ...
    fpn_t calcDistanceSumNotOneToOne(const board0_t& bd0, const board1_t& bd1, int *const pdist = nullptr){
        double sum = 0;
        iterate(BitSet32(bd0.sb() & (~bd1.sb())), [bd0, pdist, &sum](uint32 idx)->void{
        ...
        iterate(BitSet32(bd1.sb() & (~bd0.sb())), [bd1, pdist, &sum](uint32 idx)->void{
        ...
        iterate(BitSet32(bd0.sb() & bd1.sb()), [bd0, bd1, pdist, &sum](uint32 idx)->void{
            double d = calcDistance(bd0.stone(idx), bd1.stone(idx));
            sum += d;
            if(pdist != nullptr){
                int l = int(d * 1000);
    ...
    void interpolate(const board0_t& bd0, const board1_t& bd1, board2_t *const pbd)noexcept{
        pbd->clearStones();
        iterateStoneWithIndex(bd0, [&](uint32 idx, const auto& st){
    ...
    public:
        ...
        stone_t& stone(uint32 idx){ return st_[idx]; }
        stone_t& stone(Color c, uint32 idx){ return st_[CItoTurn(c, idx)]; }
        
        const stone_t& stone(uint32 idx)const{ return st_[idx]; }
        const stone_t& stone(Color c, uint32 idx)const{ return st_[CItoTurn(c, idx)]; }
        ...
        StoneState state(uint32 idx)const{
            StoneState state;
            state.init();
            state.setIndex(idx);
        ...
        template<class st_t>
        void locateStone(uint32 idx, const st_t& ast){
            // 指定した番号の石を置く(前からその番号の石があるかもしれない)
            stone(idx).set(ast.x, ast.y);
            sb_.set(idx);
        }
        template<class st_t>
        void locateNewStone(uint32 idx, const st_t& ast){
            // 指定した番号の新しい石を置く
            stone(idx).set(ast.x, ast.y);
            sb_.set(idx);
        }
        template<class st_t>
        void relocateStone(uint32 idx, const st_t& ast){
            // すでに置かれていた石の位置を変更する
            ASSERT(sb().test(idx),);
            stone(idx).set(ast.x, ast.y);
        }
        void removeStone(uint32 idx)noexcept{
            ASSERT(sb().test(idx),);
            sb_.reset(idx);
        }
    ...
    int searchNo1Index(const MiniBoard& bd, Color c)noexcept{
        int minidx = -1;
        fpn_t mind2(10000);
        iterateStoneWithIndex(bd, c, [&minidx, &mind2](uint32 idx, const auto& st)->void{
    ...
    int searchNo1Index(const MiniBoard& bd)noexcept{
        int minidx = -1;
        fpn_t mind2(10000);
        iterateStoneWithIndex(bd, [&minidx, &mind2](uint32 idx, const auto& st)->void{
            double d2 = calcDistance2Tee(st);
            if(d2 < mind2){
                minidx = idx;
                mind2 = d2;
    ...
    int countScore(const MiniBoard& bd)noexcept{
        ...
        iterateStoneWithIndex(bd, BLACK, [&bmin2, &r2a](uint32 idx, const auto& st)->void{
            fpn_t r2 = calcDistance2Tee(st);
            r2a[idx] = r2;
            bmin2 = min(bmin2, r2);
        });
        iterateStoneWithIndex(bd, WHITE, [&wmin2, &r2a](uint32 idx, const auto& st)->void{
            fpn_t r2 = calcDistance2Tee(st);
            r2a[idx] = r2;
            wmin2 = min(wmin2, r2);
        });
        if (bmin2 == wmin2){
            ...
            int cnt = 0;
            iterateStoneWithIndex(bd, WHITE, [bmin2, &r2a, &cnt](uint32 idx, const auto& st)->void{
                if (r2a[idx] < bmin2){ ++cnt; }
            });
            return -cnt;
        }else{
            int cnt = 0;
            iterateStoneWithIndex(bd, BLACK, [wmin2, &r2a, &cnt](uint32 idx, const auto& st)->void{
    ...
    public:
        constexpr static uint64 CHANGED_FLAG = (1ULL << 63); // 盤上に変化あり(draw含む)
        constexpr static uint64 COMPLEX_FLAG = (1ULL << 61); // 複雑な衝突があったので記録が信用できない
        constexpr static uint64 ERROR_FLAG = (1ULL << 60); // エラー
        
        constexpr static uint64 PASS = 0ULL;
        constexpr static uint64 PUT = CHANGED_FLAG;
        ...
        constexpr ContactTree(uint64 arg) : BitArray64<4, 15>(arg){}
        ...
        constexpr uint64 isChanged()const noexcept{ return (data() & CHANGED_FLAG); }
        constexpr uint64 isComplex()const noexcept{ return (data() & COMPLEX_FLAG); }
        constexpr uint64 isError()const noexcept{ return (data() & ERROR_FLAG); }
