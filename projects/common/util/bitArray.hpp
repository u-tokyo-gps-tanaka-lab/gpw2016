...
void iterateAll(BitArrayInRegister<data_t, MAX_SIZE, N, SIZE, atomic_data_t> ba, const callback_t& callback)noexcept{ // 全て
    for (int i = 0;; ++i){
        callback(ba.front());
        if (i == SIZE - 1){ break; }
        ba.remove_front();
...
template<typename data_t, int MAX_SIZE, int N, int SIZE, typename atomic_data_t, typename callback_t>
void iterateAny(BitArrayInRegister<data_t, MAX_SIZE, N, SIZE, atomic_data_t> ba, const callback_t& callback)noexcept{ // 以降0なら終了
    while (ba.any()){
        callback(ba.front());
        ba.remove_front();
    }
...
template<typename data_t, int MAX_SIZE, int N, int SIZE, typename atomic_data_t, typename callback_t>
void iterateAnyWithoutFirstCheck(BitArrayInRegister<data_t, MAX_SIZE, N, SIZE, atomic_data_t> ba, const callback_t& callback)noexcept{ // 以降0なら終了, 全0の判定なし
    do{
        callback(ba.front());
        ba.remove_front();
    } while (ba.any());
...
using BitArray8 = BitArrayInRegister<uint8, 8, N, SIZE>;

template<int N = 4, int SIZE = 16 / N>
using BitArray16 = BitArrayInRegister<uint16, 16, N, SIZE>;

template<int N = 4, int SIZE = 32 / N>
using BitArray32 = BitArrayInRegister<uint32, 32, N, SIZE>;

template<int N = 4, int SIZE = 64 / N>
using BitArray64 = BitArrayInRegister<uint64, 64, N, SIZE>;
