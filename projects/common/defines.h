...
// タイムスタンプ読み込みにかかる時間
// 環境依存しそうなので0にしておく
constexpr uint64 CLOCK_LATENCY = 0ULL;
...
public:
    ...
    uint64 stop()const noexcept{ return cputime() - c_start - CLOCK_LATENCY; }
    uint64 restart()noexcept{ // 結果を返し、0から再スタート
        uint64 tmp = cputime();
        uint64 old = c_start;
        c_start = tmp;
